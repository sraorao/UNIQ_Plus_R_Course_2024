...
# Test if an expression is true or not (Boolean expression) 
```{r}
...
# Let´s define a funtion that does what mean() does
...
# Loop functions
Functions such as `lapply`, `sapply` and `mapply` are a different way of running a for loop. They use another function as an argument, which can be an existing 'named' function or an 'anonymous' function.
```{r}
